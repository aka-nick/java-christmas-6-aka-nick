...
- config: 실행흐름과 무관한 정적인 설정 정보를 가지고 있는 패키지.
- promotion: 실행할 어플리케이션 로직 패키지.
  - organizer: '주도자'로서, 실행 흐름을 핸들링하는 책임들의 패키지.
    - manager: 실행 흐름의 여러 맥락 중 하나의 맥락을 담당해서 책임지는 '담당자' 패키지.
    - io: organizer(정확히는 manager)가 제공받을 i/o 관련 인터페이스 패키지.
  - collaborator: 메인 실행흐름의 요청에 맞추어 협력할 협력객체들의 패키지
    - calendar: 이벤트달력 구성과 기능 책임
    - generic: 보편 책임
    - menu: 메뉴 구성에 대한 책임
    - order: 주문과 관련한 기능 책임
    - promotions: 사용자가 받게 될 이벤트 혜택에 관한 책임
  - enums: 열거 데이터나 상수와 같이 정적인 정보를 모아 제공하는 enum 패키지. 어플리케이션 내 공통적으로 사용되는 GlobalMessage를 제외한 나머지의 구조는 enums를 제외한 promotion 내 패키지 구조를 그대로 따라가기 때문에, enums 이하 구조 설명은 생략한다.
  - exception: 커스텀 예외 책임 패키지
  - io: i/o 인터페이스의 구현 책임을 가지고 있는 패키지. 외부 의존성을 가지고 있을 확률이 높다.
...
- [x] 비공개 저장소로 만들었는가
- [x] 저장소명 뒤에 깃허브 아이디를 적었는가
- [x] 우아한 테크코스 계정을 collaborator로 초대했는가
- [x] main 브랜치에서 작업하고 있는가

#### 프로젝트 관련

- [x] 요구사항에 명시된 출력값 형식을 지켰는가
- [x] Java 17 버전인가? (`java -version`)
- [x] 테스트에 모두 성공했는가? 성공함을 확인했는가? (`./gradlew clean test`)
